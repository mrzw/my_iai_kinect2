 ...
 */
// 基于iai_kinect2/kinect2_viewer/viewer.cpp修改，在原有基础上实现按b开始连续录制，e停止录制，并修改保存图片函数saveCloudAndImages。
#include <stdlib.h>
#include <stdio.h>
#include <iostream>
#include <sstream>
...
private:
  ...
  std::ofstream out_file;  // zw 添加

public:
  Receiver(const std::string &topicColor, const std::string &topicDepth, const bool useExact, const bool useCompressed)
    : topicColor(topicColor), topicDepth(topicDepth), useExact(useExact), useCompressed(useCompressed),
      updateImage(false), updateCloud(false), save(false), running(false), frame(0), queueSize(5),
      nh("~"), spinner(0), it(nh), mode(CLOUD), save_seq(false), out_file("/home/zw/github/catkin_ws/src/iai_kinect2/zw_data/associate.txt")
      // save_seq out_file是新添加的
  {
    ...
    if(!out_file)
    {
      std::cerr<<"failed open associate.txt"<<std::endl;
    }
    //////////////////////////////////////
  }
...
/////////////////////////////////////
  ...
  void cloudViewer()
  {
    ...
    for(; running && ros::ok();)
    {
      ...
      if(save || save_seq)  // save_seq是新添加的
      {
        save = false;
        cv::Mat depthDisp;
        dispDepth(depth, depthDisp, 12000.0f);
...
////////////////////// zw部分修改 ////////////////////////////
    ...
    cv::Mat colorROI = color(cv::Rect(224, 8, 512, 424));
    cv::Mat depthROI = depth(cv::Rect(224, 8, 512, 424));
    cv::Mat depthColorROI = depthColored(cv::Rect(224, 8, 512, 424));

    //std::chrono::system_clock::time_point time_now = std::chrono::system_clock::now();
    //std::time_t time_now_t = std::chrono::system_clock::to_time_t(time_now);
    //cout<<"save image in time_now_t:"<<time_now_t<<endl;
    /////////////// 保存associate.txt 类似TUM /////////////
    ...
    out_file<<timestamp<<" "<<"rgb/"<<colorName<<" "<<timestamp<<" "<<"depth/"<<depthName<<endl;
    //////////////////////////////////////////////////////    
    
    OUT_INFO("saving color: " << colorName);
    cv::imwrite("rgb/"+colorName, colorROI, params);
    OUT_INFO("saving depth: " << depthName);
    cv::imwrite("depth/"+depthName, depthROI, params);
    OUT_INFO("saving depth: " << depthColoredName);
    cv::imwrite("depthColor/"+depthColoredName, depthColorROI, params);
